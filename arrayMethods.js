...
const totalWithTax = orders.map(num => num.price + num.tax)
...
const bobsTotal = purchases.filter((x) => x.owner === 'Bob').reduce((acc,curr) => acc + curr.price, 0) //first filter the object by key = owner, then reduce. accumulate key = price starting at the first price

console.log(bobsTotal)
// ((element) => element.CP > 200 );
